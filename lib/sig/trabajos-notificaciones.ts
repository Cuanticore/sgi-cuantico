...
// lib/sig/trabajos-notificaciones.ts
//
// El nucleo de los envios: los cinco tipos de aviso y los dos resumenes.
//
// Vivia en `app/sig/acciones/envios.ts` detras de `autorConPermiso`, y por eso el cron no
// podia llamarlo: sin sesion la compuerta devuelve `ok: false` y el trabajo pareceria
// haber corrido. Movido aca con `server-only`, el autor entra como parametro y la
// compuerta se queda en la accion, que es la que tiene una persona detras.
//
// No se puede poner en un archivo `use server`: ahi toda exportacion se vuelve una
// server action invocable desde el navegador, y esta funcion escribe sin preguntar quien
// es. Mismo patron que `lib/sgsi/directorio.ts` y `lib/sig/trabajos.ts`.
...
import type { Resultado } from '@/app/sgsi/acciones/sesion';
...
export async function enviarNotificacionesComo(autor: string): Promise<ResultadoEnvios> {
  {
    ...
    // El autor no se usa en el cuerpo: esta función no escribe bitácora, sólo envía y
    // registra `EnvioNotificacion`. Se recibe igual para que la firma sea la misma que la
    // de los demás trabajos, y para no tener que cambiarla el día que sí haga falta.
    void autor;
