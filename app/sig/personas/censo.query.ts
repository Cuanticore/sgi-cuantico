...
// app/sig/personas/censo.query.ts
//
// El censo: las personas con su rol derivado de los grupos (la aplicación no guarda roles),
// su estado, sus pendientes abiertos y los catálogos que el popup de edición necesita.
//
// VIVE APARTE DE LA PÁGINA PORQUE AHORA LO LEEN DOS. `/sig/personas` sigue siendo la vista
// del censo —de dónde sale quién existe y cuándo se sincronizó—, y `/sig/colaboradores` es
// la integradora, que necesita exactamente las mismas filas para poder abrir el popup de
// edición sobre cualquiera de ellas.
//
// Duplicar este mapeo en las dos pantallas habría sido la forma segura de que dentro de dos
// meses una mostrara un rol que la otra no: son las mismas personas y tienen que salir de la
// misma lectura.
...
export async function cargarCenso(): Promise<Censo> {
