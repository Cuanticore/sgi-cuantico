...
Hoy «área» hace de dos cosas a la vez. El caso que lo zanja es real: **la Gerencia de Operaciones, en el área Operaciones, responde por tres procesos —Proyectos, Soporte y Servicio al Cliente, y Gestión Tecnológica—, dos misionales y uno de apoyo.** Con una sola entidad ese hecho no se puede escribir.
...
model Proceso {
  id       Int
  codigo   String  @unique   // EST, COM, PRO, SAC, TAL, LCO, TEC, SIG, FIN
  nombre   String
...
| `COM` | Misional | Gestión Comercial | Gerencia Comercial | Lina Medina |
| `PRO` | Misional | Gestión de Proyectos | Gerencia de Operaciones | Yuliet Rojas |
| `SAC` | Misional | Soporte y Servicio al Cliente | Gerencia de Operaciones | Yuliet Rojas |
...
| `TEC` | Apoyo | Gestión Tecnológica | Gerencia de Operaciones | Yuliet Rojas |
...
Ambas son errores míos al escribir la pantalla y este documento; `configuracion-base-tareas.md` ya tenía los nueve bien separados desde el 31/08.

**Inventé un proceso que no existe.** Escribí «Tecnología y Soporte» como si fuera uno solo. En el mapa son **dos procesos distintos y de bandas distintas**: `SAC` Soporte y Servicio al Cliente es **misional**, y `TEC` Gestión Tecnológica es **de apoyo**. En la conversación diaria se mezclan; en el sistema de gestión no son lo mismo.
...
- **Un cargo, varios procesos.** Gerencia de Operaciones es dueña de `PRO`, `SAC` y `TEC`; Líder Administrativo de `TAL` y `FIN`. Seis cargos para nueve procesos, y con `Area` sola no se podía escribir.
- **Un cargo cruza las bandas.** Gerencia de Operaciones responde por dos procesos misionales y uno de apoyo. La banda es del proceso, no de quien lo lidera.
- **`TEC` es el proceso con más obligaciones de todo el sistema** —dieciocho— y el que soporta el SGSI entero. Con `PRO` y `SAC`, la Gerencia de Operaciones concentra cerca de la mitad de la carga: conviene mirarlo antes de generar las primeras asignaciones, no después.
...
2. **El programa de auditoría ya tiene filas por área.** Migrarlas exige decidir a qué proceso corresponde cada una. Es mecánico salvo donde un cargo responde por varios procesos: las filas de la Gerencia de Operaciones hay que repartirlas entre `PRO`, `SAC` y `TEC`, y las del Líder Administrativo entre `TAL` y `FIN`.
...
1. **Unificar el catálogo de cargos antes de migrar.** Con el dueño convertido en cargo, `CargoResponsable` pasa a ser una llave, y en las fuentes hay cargos escritos de varias formas: «Gerencia Comercial» / «Director de Ventas · National Sales Manager», y «Gerencia de Operaciones» / «Gerente de Operaciones · Chief Operating Officer» / «Operations & Services Manager». Cargados como cargos distintos, un proceso queda apuntando a un cargo que nadie ocupa.
2. **El área de cada proceso.** El mapa da la banda y el cargo, no el área; hay que fijarla antes de migrar.
3. **Si el área del proceso y el área del dueño pueden diferir.** El modelo dice que el proceso vive en un área; nada obliga a que quien ocupa el cargo pertenezca a esa misma. El caso de Yuliet Rojas —área Operaciones— sugiere que sí, y conviene dejarlo dicho para que nadie lo valide de más.
4. **Revisar la concentración en Gerencia de Operaciones**, que responde por tres de los nueve procesos.

> `configuracion-base-tareas.md` ya quedó corregido el 02/09/2026 con estos nueve, sus cargos y los códigos unificados.
